CLASS lsc_zbp_i_online_shop_c23 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS save_modified REDEFINITION.
ENDCLASS.

CLASS lsc_zbp_i_online_shop_c23 IMPLEMENTATION.

  METHOD save_modified.
    DATA : lt_online_shop_as TYPE STANDARD TABLE OF zshop_as_c23,
           ls_online_shop_as TYPE                   zshop_as_c23.
    IF zbp_i_online_shop_c23=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.
      LOOP AT zbp_i_online_shop_c23=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
    ...
    IF create-online_shop IS NOT INITIAL.
      ...
      INSERT zshop_as_c23 FROM TABLE @lt_online_shop_as.
    ENDIF.
  ENDMETHOD.
ENDCLASS.

CLASS lhc_zbp_i_online_shop_c23  DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_zbp_i_online_shop_c23  IMPLEMENTATION.
...
*  if a new package is ordered, trigger a new purchase requisition
      ...
      WITH VALUE #( ( %cid_ref = 'My%ItemCID_1'
                      %target  = VALUE #( (
                                          ...
                                          textobjecttype = 'B01'
                                          language       = 'E'
                                          plainlongtext  = 'item text created from PAAS API c23'
                                        ) (
                                          %cid = 'My%CCT_2'
                                          textobjecttype = 'B02'
                                          language       = 'E'
                                          plainlongtext  = 'item2 text created from PAAS API c23'
                                        ) )
              )   )
      ...
      zbp_i_online_shop_c23=>cv_pr_mapped = ls_pr_mapped.
    ENDLOOP.
...
* Update an existing info record
  ...
  METHOD calculate_order_id.
    DATA:
      online_shops TYPE TABLE FOR UPDATE zi_online_shop_c23,
      online_shop  TYPE STRUCTURE FOR UPDATE zi_online_shop_c23.

    SELECT MAX( order_id ) FROM zonlineshop_c23 INTO @DATA(max_order_id).

    READ ENTITIES OF zi_online_shop_c23 IN LOCAL MODE
       ENTITY Online_Shop
        ALL FIELDS
    ...
    MODIFY ENTITIES OF zi_online_shop_c23 IN LOCAL MODE
    ENTITY Online_Shop UPDATE SET FIELDS WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  zi_online_shop_c23~create_pr,
